...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if ((0X00B5 | 0X8000) == cId || (0X00B4 | 0X8000) == cId)
	{
		...
		for (i = 0; i < 8; i++)
		{
			if (pbuf[28] & (1 << i))		printf("P%d_%d=3-State\n", 0, i);
			else						printf("P%d_%d=Pullup/pulldown\n", 1, i);
		}
		return;
	}
...
int PackDesc(const unsigned short cId, unsigned char *pbuf)
{
	...
	if (WHOAMI_REQ_DESC == cId || NTEMP_REQ_DESC == cId || 0X001B == cId || 0X00B5 == cId)
		return 5;
	if (PWM_SET_DESC == cId)
		return 7;
	if (PWM_REQ_DESC == cId)
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(0X00B5, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, 0X00B5, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("P1INPReq (0X0124,0X00B5): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for P1INP_RSP (0X80B5,0X80B4) TIMEOUT !\n\n", t);

...
int cgiMain()
{
	...
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) cid not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (0 != strcmp(temp, "0X0019"))
	{
...
// pan check
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("pan", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) pan not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
		res = 1, fprintf(cgiOut, "(%s %d) pan error !!", __FILE__, __LINE__);
		goto END;
	}
	if (6 != strlen(temp))
	{
...
// addr check
	...
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) IEEE address not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
		res = 1, fprintf(cgiOut, "(%s %d) IEEE address error !!", __FILE__, __LINE__);
		goto END;
	}
	if (18 != strlen(temp))
	{
	...
	res = sscanf(temp, "%LX", &addr);
	if (-1 == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) IEEE address error !!", __FILE__, __LINE__);
		goto END;
	}
	addr &= 0XFFFFFFFFFFFFFFFFLLU;
...
// txopts check
	...
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) txopts not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
		res = 1, fprintf(cgiOut, "(%s %d) txopts error !!", __FILE__, __LINE__);
		goto END;
	}
	if (4 != strlen(temp))
	{
...
// radius check
	...
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) radius not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) > 2)
	{
		res = 1, fprintf(cgiOut, "(%s %d) radius error !!", __FILE__, __LINE__);
		goto END;
	}
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("delay", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) delay not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) > 2)
	{
		res = 1, fprintf(cgiOut, "(%s %d) delay error !!", __FILE__, __LINE__);
		goto END;
	}
	delay = atoi(temp);
	if (delay < 0 || delay > 60)
...
/////////////////////////////
// 
	if (0 == (l = PackDesc(RESET_REQ_DESC, frame)))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!", __FILE__, __LINE__);
		goto END;
	}
	memcpy(&frame[5], &delay, 2);

	if (0 == PackAfExtCmd(3, addr, (unsigned short)pan, RESET_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)l, frame))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!", __FILE__, __LINE__);
		goto END;
	}

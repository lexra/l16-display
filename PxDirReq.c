...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(PXDIR_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;


	if (0 == PackAfCmd((unsigned short)addr, PXDIR_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PxDirReq (0X0124,0X0038): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u\n", addr, radius, radius, txopts, ioport);

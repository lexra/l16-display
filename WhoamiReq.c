...
void ProcessWhoamiRsp1(unsigned char descLen, unsigned char *pdesc, unsigned int latency)
{
	...
	if (0 != lqi && 255 != lqi)
		printf("[%02us] WHOAMI_RSP (0X8001), [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64" [PA]=0X%04X \n[PROFILE_ID]=0X%04X [PAN_ID]=0X%04X [STACK_PROFILE]=%s [CH-LIST]=%s [TYPE]=%s \n[LQI]=0X%02X,%u [RSSI]=%d \n", 
			latency, my, my, sa, ia, pa, prof, pan, SPF, lst, type, lqi, lqi, (char)rssi);
	else
		printf("[%02us] WHOAMI_RSP (0X8001), [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64" [PA]=0X%04X \n[PROFILE_ID]=0X%04X [PAN_ID]=0X%04X [STACK_PROFILE]=%s [CH-LIST]=%s [TYPE]=%s \n[LQI]=N/A [RSSI]=N/A \n", 
			latency, my, my, sa, ia, pa, prof, pan, SPF, lst, type);
}
...
void ProcessAfIncoming1(unsigned short cmd, unsigned char *pbuf, unsigned int latency)
{
	...
	{
		ProcessWhoamiRsp1(DataLen, &pbuf[21], latency);
		return;
	}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -T [timeout]\n", filename);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -t9\n", filename), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	printf("WhoamiReq (0X0124,0X0001): [SA]=0X%04X [RADIUS]=0X%02X [TxOpts]=0X%02X [SN]=%03u,0X%02X\n", addr, radius, txopts, frame[10], frame[10]);
	...
	do
	{
			...
			if ((int)(now - first) > t)
			{
				if (0XFFFF == addr)
				{
					...
					for (i = 0; i < 512; i++)
					{
						...
						unsigned short x2, x3, x4;
						unsigned char x5, x6;

						memcpy(&x1, &nodes[16 * i + 0], sizeof(unsigned long long int));
						if (0XFFFFFFFFFFFFFFFFLLU == x1)
							break;	//continue;
						memcpy(&x2, &nodes[16 * i + 0 + 8], sizeof(unsigned short));
						memcpy(&x3, &nodes[16 * i + 0 + 8 + 2], sizeof(unsigned short));
						memcpy(&x4, &nodes[16 * i + 0 + 8 + 2 + 2], sizeof(unsigned short));
						memcpy(&x5, &nodes[16 * i + 0 + 8 + 2 + 2 + 2], sizeof(unsigned char));
						memcpy(&x6, &nodes[16 * i + 0 + 8 + 2 + 2 + 2 + 1], sizeof(unsigned char));

						if (0 == x5)
						{
							if (0 == x6)	printf("NODE(%03d) -> [IA]=0X%016"PRIX64" [SA]=0X%04X [PA]=0X%04X [MY]=%05u,0X%04X [TYPE]=C [LQI]=N/A\n", i, x1, x2, x3, x4, x4);
							else			printf("NODE(%03d) -> [IA]=0X%016"PRIX64" [SA]=0X%04X [PA]=0X%04X [MY]=%05u,0X%04X [TYPE]=C [LQI]=%03u [RSSI]=%d\n", i, x1, x2, x3, x4, x4, x6, (char)((97.00*(float)x6/255.00) - 87.00));
						}
						else if (1 == x5)
						{
							if (0 == x6)	printf("NODE(%03d) -> [IA]=0X%016"PRIX64" [SA]=0X%04X [PA]=0X%04X [MY]=%05u,0X%04X [TYPE]=R [LQI]=N/A\n", i, x1, x2, x3, x4, x4);
							else			printf("NODE(%03d) -> [IA]=0X%016"PRIX64" [SA]=0X%04X [PA]=0X%04X [MY]=%05u,0X%04X [TYPE]=R [LQI]=%03u [RSSI]=%d\n", i, x1, x2, x3, x4, x4, x6, (char)((97.00*(float)x6/255.00) - 87.00));
						}
						else
						{
							if (0 == x6)	printf("NODE(%03d) -> [IA]=0X%016"PRIX64" [SA]=0X%04X [PA]=0X%04X [MY]=%05u,0X%04X [TYPE]=D [LQI]=N/A\n", i, x1, x2, x3, x4, x4);
							else			printf("NODE(%03d) -> [IA]=0X%016"PRIX64" [SA]=0X%04X [PA]=0X%04X [MY]=%05u,0X%04X [TYPE]=D [LQI]=%03u [RSSI]=%d\n", i, x1, x2, x3, x4, x4, x6, (char)((97.00*(float)x6/255.00) - 87.00));
						}
		...
		if (addr == 0XFFFF)
		{
			if (cmd == 0X8144)
			{
				...
				memcpy(&ieee, &rb[21 + 9], sizeof(unsigned long long int));
				for (i = 0; i < 512; i++)
				{
					memcpy(&a, &nodes[16 * i], sizeof(unsigned long long int));
					if (a == ieee)
					{
						f = 1;
						s = i;
				...
				if (0 == f)
				{
					...
					if (1 == e)
					{
						...
						memcpy(&nodes[16 * i + 0], &rb[21 + 9], sizeof(unsigned long long int));
						memcpy(&nodes[16 * i + 0 + 8], &rb[21 + 7], sizeof(unsigned short));
						memcpy(&nodes[16 * i + 0 + 8 + 2], &rb[21 + 17], sizeof(unsigned short));
						memcpy(&nodes[16 * i + 0 + 8 + 2 + 2], &rb[21 + 5], sizeof(unsigned short));
						n = rb[21 + 19];
						if (2 != n)	printf("n != 2\n");
						nodes[16 * i + 0 + 8 + 2 + 2 + 2] = rb[21 + 19 + n + 10];	// TYPE

						if (rb[13] > rb[21 + 19 + n + 11] && 255 != rb[13])
							m = rb[13];
						else
						{
							if (255 != rb[21 + 19 + n + 11])						m = rb[21 + 19 + n + 11];
						}

						if (nodes[16 * i + 0 + 8 + 2 + 2 + 2 + 1] == 255)			nodes[16 * i + 0 + 8 + 2 + 2 + 2 + 1] = m;
						else
						{
							if (m > nodes[16 * i + 0 + 8 + 2 + 2 + 2 + 1])			nodes[16 * i + 0 + 8 + 2 + 2 + 2 + 1] = m;
						}
					}
				}
				...
				{
					//if (rb[13] != 255 && s != -1)
					//	if (rb[13] > nodes[16 * s + 0 + 8 + 2 + 2 + 2 + 1])
					//		nodes[16 * s + 0 + 8 + 2 + 2 + 2 + 1] = rb[13];

					n = rb[21 + 19];
					if (2 != n)	printf("n != 2\n");
					if (rb[21 + 19 + n + 11] != 255 && s != -1)
						if (rb[21 + 19 + n + 11] > nodes[16 * s + 0 + 8 + 2 + 2 + 2 + 1])
							nodes[16 * s + 0 + 8 + 2 + 2 + 2 + 1] = rb[21 + 19 + n + 11];
				}

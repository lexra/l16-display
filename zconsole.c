...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		if (cmd == 0X9345)
		{
			...
			GetLocalTime(&st);
			printf("[%04d-%02d-%02d %02d:%02d:%02d %010u] ", st.tm_year + 1900, st.tm_mon + 1, st.tm_mday, st.tm_hour, st.tm_min, st.tm_sec, (unsigned int)mktime(&st));
			printf("ZDO_END_DEVICE_ANNCE_RSP (0X%04X), [SrcAddr]=0X%04X [NwkAddr]=0X%04X [IA]=0X%016"PRIX64" [CAPABILITIES]=0X%02X ", cmd, SrcAddr, NwkAddr, ieee, Capabilites);
			if (Capabilites & (1 << 0))				printf("Bit0-ON:Alternate-PAN-Coordinator");
			if (Capabilites & (1 << 1))				printf("Bit1-ON:ZigBee-Router ");
			else									printf("Bit1-ON:End-Device ");
			if (Capabilites & (1 << 2))				printf("Bit2-ON:Main-Powered ");
			if (Capabilites & (1 << 3))				printf("Bit3-ON:Receiver-on-Idle ");
			if (Capabilites & (1 << 6))				printf("Bit6-ON:Security-Capability");
			printf("\n");
			continue;
		}

...
void ProcessGpioRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	printf("GPIO_RSP (0X%04X), [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64", port=0X%02X value=0X%02X \n", cId, my, my, sa, ieee, P, V);
	for (i = 0; i < 8; i++)
	{
		j = V & (1 << i);
		if (0 != j)	j = 1;
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -N [Pin Number] -V [High/Low (1/0)]  -t [timeout]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -N0 -V0 -t6\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:o:O:N:n:V:v:T:t:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(PIN_HI_LO_SET_DESC, frame)))
	...
	if (0 == PackAfCmd((unsigned short)addr, PIN_HI_LO_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PinHiLoSet (0X0124,0X0038): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [PIN NUMBER]=%d [VALUE]=%d\n", addr, radius, radius, txopts, ioport, pin, value);

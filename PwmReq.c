...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(PWM_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, PWM_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PwmReq (0X0124,0X0003): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);

...
#define PIN_DIR_SET_DESC						0X009B
...
#define PIN_SELECT_SET_DESC				0X00D0
...
void ProcessPxSelRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	unsigned char P;
	unsigned char V;
	...
	printf("PXSEL_RSP, [MY]=%05u,0X%04X, port=0X%02X value=0X%02X \n", my, my, P, V);
...
void ProcessPxDirRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	unsigned char P = 0;
	unsigned char V;
	...
	memcpy(&my, &pdesc[5], sizeof(unsigned short));
	printf("PXDIR_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64", value=0X%02X \n", my, my, ieee, pdesc[8]);
...
static int PackDesc(const unsigned short cId, unsigned char *pbuf)
{
	...
	if (ADC_REQ_DESC == cId)
		return 5;
	if (PIN_HI_LO_SET_DESC == cId || PIN_DIR_SET_DESC == cId)
		return 8;
...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -N [Pin Number] -V [Pheripral/Gpio (1/0)]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -N0 -V0\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:o:O:N:n:V:v:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(PIN_SELECT_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;
	frame[6] = (unsigned char)pin;
	frame[7] = (unsigned char)value;


	if (0 == PackAfCmd((unsigned short)addr, PIN_SELECT_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PinSelSet (0X0124,0X00D0): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [PIN NUMBER]=%d [VALUE]=%d\n", addr, radius, radius, txopts, ioport, pin, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for GPIO_RSP (0X%04X,0X%04X) TIMEOUT !\n\n", t, PXSEL_REQ_DESC | 0X8000, PXSEL_SET_DESC | 0X8000);

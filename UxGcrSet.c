...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X802F == cId || 0X8030 == cId)
	{
		...
		printf("UX_GCR_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64", [PORT]=%d, [VALUE]=0X%02X \n", my, my, sa, ieee, pbuf[28], pbuf[29]);
		return;
	}
...
{
	printf("Usage: UxGcrSet -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -v [value]\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X06\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X07\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X08\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X09\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X0A\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X0B\n");
	printf("Example: UxGcrSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X0C\n"), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:o:O:V:v:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'P':
			ioport = atoi(optarg);
			if (ioport < 0)							PrintUsage();
			if (ioport > 1)							PrintUsage();
			break;
		case 'p':
			port = atoi(optarg);
			break;
	...
	if (0 == (len = PackDesc(UXGCR_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;
	frame[6] = (unsigned char)value;

	if (0 == PackAfCmd((unsigned short)addr, UXGCR_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("UxBaudSet (0X0124,0X0038): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [VALUE]=0X%02X\n", addr, radius, radius, txopts, ioport, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for UXGCR_RSP (0X%04X,0X%04X) TIMEOUT !\n\n", t, UXGCR_SET_DESC | 0X8000, UXGCR_SET_DESC | 0X8000);
		...
		cId &= 0XFFFF;
		if ((UXGCR_SET_DESC | 0X8000) != cId && (UXGCR_SET_DESC | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X8018 == cId)
	{
		...
		fprintf(cgiOut, "MODBUS_QRY_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64"\n", my, my, ieee);
...
// packet + 3 * 17
	...
	frame[9] = (unsigned char)numH;
	frame[10] = (unsigned char)numL;

...
/*
static void ProcessZdoIeeeAddrRsp(unsigned short CMD, unsigned char *pdesc)
{
	...
	fprintf(cgiOut, "ZDO_IEEE_ADDR_RSP, [CMD]=0X%04X \n[status]=%d [IA]=0X%016"PRIX64" [SA]=0X%04X\n", cmd, status, ia, sa);
}
*/
...
// 
	...
	memset(temp, 0, sizeof(temp));
	memcpy(temp + 2 * 0, packet + 3 * 11, 2);
	memcpy(temp + 2 * 1, packet + 3 * 10, 2);
	memcpy(temp + 2 * 2, packet + 3 * 9, 2);
	memcpy(temp + 2 * 3, packet + 3 * 8, 2);
	memcpy(temp + 2 * 4, packet + 3 * 7, 2);
	memcpy(temp + 2 * 5, packet + 3 * 6, 2);
	memcpy(temp + 2 * 6, packet + 3 * 5, 2);
	memcpy(temp + 2 * 7, packet + 3 * 4, 2);
...
// 
	...
	do
	{
		...
		memcpy(&addr, &rb[5], sizeof(unsigned long long int));
		if (addr != ia)
		{
			fprintf(cgiOut, "(%s %d) addr != ia \naddr=0X%016"PRIX64" \nia=0X%016"PRIX64" \n", __FILE__, __LINE__, addr, ia);
			continue;
		}
		break;
	}

...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(PULSE_COUNT_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, PULSE_COUNT_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("%s (0X0124,0X%04X): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", filename, PULSE_COUNT_REQ_DESC, addr, radius, radius, txopts);
		...
		if (addr == 0XFFFF)
		{
			if (cmd == 0X8144)
			{
				...
				if (1)
				{
					...
					memcpy(&C, &rb[28], sizeof(unsigned int));

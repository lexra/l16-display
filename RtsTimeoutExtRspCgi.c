...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X80BA == cId)
	{
		...
		fprintf(cgiOut, "RTS_TIMEOUT_RSP, [MY]=0X%04X, [TIMEOUT]=%d \n", my, pbuf[28]);
		return;
	}
	...
	if (0X8018 == cId)
	{
		...
		for (i = 0; i < (int)bc + 5; i++)
		{
			...
			if (i == 0)
				fprintf(cgiOut, "[OUTPUT]: 0X%02X-", pbuf[29 + i]);
			else
				fprintf(cgiOut, "0X%02X-", pbuf[29 + i]);
		}
...
// jc note, ext
	...
	res = sscanf(packet + 3 * 28, "%X", &timeout);
	if (-1 == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) sscanf error !!\n", __FILE__, __LINE__);
		goto END;

...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if ((0X001B | 0X8000) == cId || (0X001A | 0X8000) == cId)
	{
		...
		printf("TXCTRLL_RSP (0X%04X), [SA]=0X%04X [IA]=0X%016"PRIX64" [MY]=%05u,0X%04X [Value]=%u\n", cId, sa, ieee, my, my, pbuf[28]);
		return;
	}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(0X001B, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, 0X001B, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("TxCtrllReq (0X0124,0X001B): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for TXCTRLL_RSP (0X801B,0X801A) TIMEOUT !\n\n", t);

...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(0X00B7, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, 0X00B7, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("P2INPReq (0X0124,0X00B7): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for P2INP_RSP (0X80B7,0X80B6) TIMEOUT !\n\n", t);
		...
		if ((0X00B7 | 0X8000) != cId && (0X00B6 | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

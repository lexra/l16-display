...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X80B8 == cId)
	{
		...
		printf("MODBUS_PRESET_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64"\n", my, my, sa, ieee);
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -s [Slave Address] -R [Start Register] -v [value] -t [timeout]\n", filename);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -s0X01 -R0X0000 -v0X0000 -t6\n", filename), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:s:S:v:V:o:O:T:t:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(0X00B8, frame))) {}
	frame[5] = (unsigned char)slave;
	frame[6] = (unsigned char)6;
	frame[7] = HI_UINT16((unsigned short)reg);
	frame[8] = LO_UINT16((unsigned short)reg);
	frame[9] = HI_UINT16((unsigned short)value);
	frame[10] = LO_UINT16((unsigned short)value);

	if (0 == PackAfCmd((unsigned short)addr, 0X00B8, (unsigned char)txopts, (unsigned char)radius, 11, frame))
	...
	printf("ModbusPreset (0X0124,0X00B8): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [SLAVE]=%02u [FUNC]=3 [START-REG]=0X%04X [VALUE]=0X%04X,%05u\n", addr, radius, radius, txopts, slave, reg, value, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for MODBUS_PRESET_RSP (0X%04X) TIMEOUT !\n\n", t, 0X00B8 | 0X8000);
		...
		if ((/*MODBUSQRY_REQ_DESC*/0X00B8 | 0X8000) != cId)
		{
			printf(".");
			continue;
		}
 ...
 DataLo
		...
		if (cmd == 0X8144)
		{
			memcpy(&sa, &rb[8], sizeof(unsigned short));
			sa &= 0XFFFF;
			if ((unsigned short)sa != (unsigned short)addr)
			{
				printf(".");
				continue;
			}

...
void ProcessNegTempRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	if (v > 80)
		printf("THERM_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" [TEMPERATURE]=%02.1f ¢XC\n", my, my, ieee, (float)v / 10);
	else
		printf("THERM_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" [TEMPERATURE]=%02.1f ¢XC\n", my, my, ieee, (float)v);
}
...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (PHOTO_RSP_DESC == cId)
	{
		...
		printf("PHOTO_RSP, [MY]=%05u,0X%02X [SA]=0X%04X [IA]=0X%016"PRIX64" [VALUE]=%05u\n", my, my, sa, ieee, BUILD_UINT16(pbuf[28], pbuf[29]));
		return;
	}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(PHOTO_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, PHOTO_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for PHOTO_RSP (0X%04X) TIMEOUT !\n\n", t, PHOTO_REQ_DESC | 0X8000);

...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [short address] -d [duration]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -d0XFF\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:a:A:D:d:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'd':
			...
			if (optarg[1] != 'X' && optarg[1] != 'x')
				PrintUsage();
			if (optarg[1] == 'X')	res = sscanf(optarg, "%X", &duration);
			else					res = sscanf(optarg, "%x", &duration);
			if (-1 == res)			PrintUsage();
			duration &= 0XFF;
			break;
	...
	frame[5] = (unsigned char)duration;
	frame[6] = 0;
	len = PackFrame(frame);
	...
	printf("ZdoMgmtPermitJoinReq (0X3625): [SHORT ADDR]=0X%04X [DURATION]=0X%02X,%u \n", addr, duration, duration);
	...
	do
	{
		...
		if ((int)(now - first) > 36)
		{
			if (0 == ack)		printf("\nWAITING 12 SECONDS for ZDO_NWK_ADDR_RSP TIMEOUT !\n\n"), close(sd), exit(1);
			else				printf("\n"), close(sd), exit(0);
		}
		...
		memcpy(&sa, &rb[4], sizeof(unsigned short)), sa &= 0XFFFF;
		if (addr != 0XFFFF && addr != 0XFFFD && addr != 0XFFFC)
		{
			if (sa != addr)	continue;
			ack = 1;
			printf("\n");
			...
			printf("ZDO_MGMT_PERMIT_JOIN_RSP (0XB645) , [Status]=%s [SA]=0X%04X\n", (rb[6] == 0) ? "SUCCESS" : "FAIL", BUILD_UINT16(rb[4], rb[5]) );
			printf("\n");
			break;
		}
		ack = 1;

...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Nwk address] -I [IEEE address] -C [Capabilites]\n", file);
	printf("DEV Example: %s -hlocalhost -p34000 -a0X796F -I0X00124B0001142FD0 -C0X04 \n", file);
	printf("RTR Example: %s -hlocalhost -p34000 -a0X0001 -I0X00124B0001142FD1 -C0X14 \n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	int cap = 0X04;
	int addr = 0X796F;
	int SrcAddr = 0, NwkAddr = 0, Capabilites = 0;
	...
	while ((c = getopt(argc, argv, "h:p:I:c:C:a:A:")) != -1)
	{
		switch (c)
		{
		default:
	...
	memcpy(&frame[5], &ia, sizeof(unsigned long long int));
	...
	printf("ZdoEndDeviceAnnce (0X0A25): [NwkAddr]=0X%04X [IA]=0X%016"PRIX64" [CAP]=0X%02X\n", addr, ia, cap);
	...
	do
	{
		...
		time(&now);
		if ((int)(now - first) > 3)
			printf("\nWAITING 3 SECONDS for ZDO_END_DEVICE_ANNCE_SRSP TIMEOUT !\n\n"), close(sd), exit(1);
	...
	if (0 != rb[4])
		printf("\nZDO_END_DEVICE_ANNCE_SRSP (0X0A65), FAIL !\n\n"), close(sd), exit(1);
	printf("\nZDO_END_DEVICE_ANNCE_SRSP (0X0A65), SUCCESS !\n\n");
	...
	do
	{
		...
		time(&now);
		if ((int)(now - first) > 12)
			printf("\nWAITING 12 SECONDS for ZDO_END_DEVICE_ANNCE_IND (0X9345), TIMEOUT !\n\n"), close(sd), exit(1);
		...
		memcpy(&ieee, &rb[8], sizeof(unsigned long long int));
		if (ieee != ia)
		{
			printf(".");
			continue;
		...
		memcpy(&SrcAddr, &rb[4], sizeof(unsigned short));
		memcpy(&NwkAddr, &rb[6], sizeof(unsigned short));
		Capabilites = rb[16];
	...
	printf("\nZDO_END_DEVICE_ANNCE_IND (0X9345), [SrcAddr]=0X%04X [NwkAddr]=0X%04X [IA]=0X%016"PRIX64" [Capabilities]=0X%02X!\n", SrcAddr, NwkAddr, ieee, Capabilites);

	if (Capabilites & (1 << 0))
		printf("Capabilites Bit-0 ON: Alternate PAN Coordinator\n");
	if (Capabilites & (1 << 1))
		printf("Capabilites Bit-1 ON: ZigBee Router\n");
	else
		printf("Capabilites Bit-1 ON: End Device\n");
	if (Capabilites & (1 << 2))
		printf("Capabilites Bit-2 ON: Main powered\n");
	if (Capabilites & (1 << 3))
		printf("Capabilites Bit-3 ON: Receiver on when idle\n");
	if (Capabilites & (1 << 6))
		printf("Capabilites Bit-6 ON: Security capability\n");

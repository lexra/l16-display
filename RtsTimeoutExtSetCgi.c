...
// radius check
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("timeout", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) timeout not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) > 2)
	{
		res = 1, fprintf(cgiOut, "(%s %d) timeout error !!\n", __FILE__, __LINE__);
		goto END;
	}
	timeout = atoi(temp);
...
// 
	...
	if (0 == PackAfExtCmd(3, addr, (unsigned short)pan, 0X00BA, (unsigned char)txopts, (unsigned char)radius, (unsigned char)l, frame))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!\n", __FILE__, __LINE__);
		goto END;
	}

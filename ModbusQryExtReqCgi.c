...
// pan check
	...
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) pan not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
		res = 1, fprintf(cgiOut, "(%s %d) pan error !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (6 != strlen(temp))
	{
...
// addr check
	...
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) IEEE address not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
		res = 1, fprintf(cgiOut, "(%s %d) IEEE address error !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (18 != strlen(temp))
	{
...
// radius check
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("slave", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) slave not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) > 2)
	{
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("fn", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) fn not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) > 2)
	{
		res = 1, fprintf(cgiOut, "(%s %d) fn error !!\n", __FILE__, __LINE__);
		goto END;
	}
	fn = atoi(temp);
	if (fn != 3 && fn != 5)
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("start", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) start not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("num", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) num not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (temp[0] != '0' || temp[1] != 'X')
	{
		res = 1, fprintf(cgiOut, "(%s %d) num error !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (6 != strlen(temp))
	{
...
// 
	...
	frame[7] = HI_UINT16((unsigned short)start);
	frame[8] = LO_UINT16((unsigned short)start);
	frame[9] = HI_UINT16((unsigned short)num);
	frame[10] = LO_UINT16((unsigned short)num);

	if (0 == PackAfExtCmd(3, addr, (unsigned short)pan, MODBUSQRY_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)l, frame))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!\n", __FILE__, __LINE__);
		goto END;
	}

...
int PackDesc(const unsigned short cId, unsigned char *pbuf)
{
	...
	if (WHOAMI_REQ_DESC == cId || NTEMP_REQ_DESC == cId || 0X001B == cId || 0X00B3 == cId)
		return 5;
	if (PWM_SET_DESC == cId)
		return 7;
	if (PWM_REQ_DESC == cId)
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(0X00B3, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, 0X00B3, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("P0INPReq (0X0124,0X00B3): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for P0INP_RSP (0X80B3,0X80B2) TIMEOUT !\n\n", t);

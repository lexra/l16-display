...
static void ProcessAdcRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	n = pdesc[7];
	p = &pdesc[8];
	printf("ADC_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" \n", my, my, ieee);
	for (; i < n; i++)
	{
		short v;
...
// packet + 3 * 17
	...
	if (0 == PackAfCmd(sa, ADC_REQ_DESC, (unsigned char)op, (unsigned char)ra, (unsigned char)l, frame))
	{

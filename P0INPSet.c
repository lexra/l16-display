...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if ((0X00B3 | 0X8000) == cId || (0X00B2 | 0X8000) == cId)
	{
		...
		printf("P0INP_RSP (0X%04X), [IA]=0X%016"PRIX64" [SA]=0X%04X [MY]=%05u,0X%04X [Value]=0X%02X\n", cId, ieee, sa, my, my, pbuf[28]);
...
int PackDesc(const unsigned short cId, unsigned char *pbuf)
{
	...
	if (WHOAMI_REQ_DESC == cId || NTEMP_REQ_DESC == cId || 0X001B == cId)
		return 5;
	if (PWM_SET_DESC == cId)
		return 7;
	if (PWM_REQ_DESC == cId)
	...
	if (GPIO_SET_DESC == cId)
		return 7;
	if (GPIO_REQ_DESC == cId || 0X001A == cId || 0X00B2 == cId)
		return 6;
	if (ADC_REQ_DESC == cId)
		return 5;
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(0X00B2, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &value, 1);

	if (0 == PackAfCmd((unsigned short)addr, 0X00B2, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("%s (0X0124,0X00B2): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [Value]=0X%02X\n", filename, addr, radius, radius, txopts, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for P0INP_RSP (0X80B2,0X80B3) TIMEOUT !\n\n", t);
		...
		if ((0X00B2 | 0X8000) != cId && (0X00B3 | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

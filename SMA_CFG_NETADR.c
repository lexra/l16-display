...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -S [Source] -D [Destination] -s [SN] -N [NewAddr]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X796F -r30 -o0X20 -S0X0001 -D0X0002 -s9380933 -N0X0003\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	unsigned char user[8] = {0,0,0,0,0,0,0,0};
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:s:S:d:D:n:N:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'N':
			...
			if ('X' == optarg[1])							res = sscanf(optarg, "%X", &NA);
			else											res = sscanf(optarg, "%x", &NA);
			if (-1 == res)									PrintUsage();
			NA &= 0XFFFF;
			break;
		case 's':
			...
			{
				if (optarg[i] < 0X30)						PrintUsage();
				if (optarg[i] > 0X39)						PrintUsage();
			}
			osn = atoi(optarg);
			break;
		case 'S':
...
//
	...
	memcpy(&user[0], &osn, sizeof(unsigned int));
	memcpy(&user[4], &NA, sizeof(unsigned short));
	len = MkSmaDataTelegram(telegram, src, dst, 0X80, 0X00, 0X03, 6, user);
	...
	printf("SMA_CFG_NETADDR (0X0124,0X00EA): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TXOPTS]=0X%02X [SRC]=0X%04X [DST]=0X%04X\n", addr, radius, radius, txopts, src, dst);
...
//
	...
	do
	{
		...
		{
			if ((int)(now - first) > t)							printf("\nWAITING %d SECONDS for SMA_CFG_NETADDR_RSP (0X80E2) TIMEOUT !\n\n", t), 	close(sd), exit(1);
		}
		FD_ZERO(&rset);
		FD_SET(sd, &rset);
		...
		// fcs
		//memcpy(&cs, &rb[29 + 19 + 4 + offs], sizeof(unsigned short));
		cs = BUILD_UINT16(rb[29 + 19 + 5 + offs], rb[29 + 19 + 4 + offs]);
		...
		memcpy(&nsn, &rb[40 + offs], sizeof(unsigned int));
		if (osn != nsn)
		{
			printf(".");
			continue;
	...
	printf("\nSMA_CFG_NETADDR_RSP (0X%04X), [IA]=0X%016"PRIX64" [SA]=0X%04X [MY]=%05u,0X%04X [DEV-ADDR]=0X%04X [SN]=0X%08X\n", cId, ieee, sa, my, my, dev, nsn);

...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X80BA == cId)
	{
		...
		printf("RTS_TIMEOUT_RSP (0X%04X) , [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" [TIMEOUT]=%d \n", cId, my, my, ieee, pbuf[28]);
...
static void PrintUsage(void)
{
	...
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -t12\n", filename), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:t:T:o:O:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 't':
			...
			{
				if ('X' == optarg[1])				res = sscanf(optarg, "%X", &timeout);
				else								res = sscanf(optarg, "%x", &timeout);
			}
			else
			{
				timeout = atoi(optarg);
	...
	if (0 == (len = PackDesc(0X00BA, frame))) //{}
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)timeout;

	if (0 == PackAfCmd((unsigned short)addr, 0X00BA, (unsigned char)txopts, (unsigned char)radius, 11, frame))
	...
	printf("RtsTimeoutSet (0X0124,0X00BA): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [TIMEOUT]=%02u\n", addr, radius, radius, txopts, timeout);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for RTS_TIMEOUT_RSP (0X%04X) TIMEOUT !\n\n", t, 0X00BA | 0X8000);

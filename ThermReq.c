...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(NTEMP_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, NTEMP_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for THERM_RSP (0X%04X) TIMEOUT !\n\n", t, NTEMP_REQ_DESC | 0X8000);

...
void ProcessJboxRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	printf("JBOX_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64" \n[DC VOLTAGE]=%05u \n[DC CURRENT]=%05u \n", my, my, sa, ieee, v, a);
}
...
void ProcessNegTempRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	if (v > 80)
		printf("THERM_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64" \n[THERM TEMPERATURE]=%02.1f ¢XC\n", my, my, sa, ieee, (float)v / 10);
	else
		printf("THERM_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64" \n[THERM TEMPERATURE]=%02.1f ¢XC\n", my, my, sa, ieee, (float)v);
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(JBOX_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, JBOX_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for JBOX_RSP (0X%04X) TIMEOUT !\n\n", t, JBOX_REQ_DESC | 0X8000);

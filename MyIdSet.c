...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X8012 == cId || 0X8013 == cId)
	{
		...
		printf("NV_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64"\n", my, my, sa, ieee);

		memcpy(&attr, &pbuf[28], sizeof(unsigned short));
		printf("[ATTRIB]=0X%04X, [LEN]=%d \n", attr, pbuf[30]);
		if (pbuf[30])
		{
			printf("[DATA]=");
			for (i = 0; i < pbuf[30]; i++)
			{
				if (i == pbuf[30] - 1)	printf("0X%02X \n", pbuf[31 + i]);
				else					printf("0X%02X-", pbuf[31 + i]);
			}
		}
		return;
	}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -v [My Id]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -v0X7501\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:o:O:v:V:")) != -1)
	{
		...
		case 'v':
			if ('0' == optarg[0] && ('X' == optarg[1] || 'x' == optarg[1]))
			{
				...
				if ('X' == optarg[1])					res = sscanf(optarg, "%X", &id);
				else									res = sscanf(optarg, "%x", &id);
				if (-1 == res)							PrintUsage();
			}
			else
			{
	...
	frame[6] = 0X00;
	frame[7] = 0X02;
	memcpy(&frame[8], &id, sizeof(unsigned short));
	if (0 == len)
		len = 5 + 2 + 1 +frame[7];

	if (0 == PackAfCmd((unsigned short)addr, 0X0012, (unsigned char)txopts, (unsigned char)radius, 10, frame))
	...
	printf("MyIdSet (0X0124,0X0012/0X00D1/2): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [VALUE]=0X%04X,%u\n", addr, radius, radius, txopts, id, id);

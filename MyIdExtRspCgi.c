...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X8012 == cId || 0X8013 == cId)
	{
		...
		fprintf(cgiOut, "NV_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64"\n", my, my, ieee);
		memcpy(&attr, &pbuf[28], sizeof(unsigned short));
		fprintf(cgiOut, "[ATTRIB]=0X%04X, [LEN]=%d \n", attr, pbuf[30]);
		if (pbuf[30])
		{
			fprintf(cgiOut, "[DATA]=");
			for (i = 0; i < pbuf[30]; i++)
			{
				if (i == pbuf[30] - 1)	fprintf(cgiOut, "0X%02X \n", pbuf[31 + i]);
				else					fprintf(cgiOut, "0X%02X-", pbuf[31 + i]);
			}
		}
		return;
	}
...
// jc note, ext
	...
	{
		memset(temp, 0, sizeof(temp));
		memcpy(temp + 2 * 0, packet + 3 * 32, 2);
		memcpy(temp + 2 * 1, packet + 3 * 31, 2);
...
// packet + 3 * 17
	...
	frame[7] = 0X02;
	if (cid == 0X0012)
		memcpy(&frame[8], &value, sizeof(unsigned short));

	if (0 == PackAfCmd(sa, (unsigned short)cid, (unsigned char)op, (unsigned char)ra, (cid == 0X0012) ? 10 : 7, frame))
	{

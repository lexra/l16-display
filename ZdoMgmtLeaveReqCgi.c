...
*/
	...
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("remove", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) remove not found !!", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) != 1)
	{
		res = 1, fprintf(cgiOut, "(%s %d) remove error !!", __FILE__, __LINE__);
		goto END;
	}
	remove = atoi(temp);

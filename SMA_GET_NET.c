...
//
	...
	len = MkSmaDataTelegram(telegram, src, dst, 0X80, 0X00, 0X01, 0, 0);
	...
	printf("SMA_GET_NET (0X0124,0X00EA): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TXOPTS]=0X%02X [SRC]=0X%04X [DST]=0X%04X\n", addr, radius, radius, txopts, src, dst);
...
//
	...
	do
	{
		...
		{
			if ((int)(now - first) > t)							printf("\nWAITING %d SECONDS for SMA_GET_NET_RSP (0X80E2) TIMEOUT !\n\n", t), 	close(sd), exit(1);
		}
		FD_ZERO(&rset);
		FD_SET(sd, &rset);
	...
	printf("\nSMA_GET_NET_RSP (0X%04X), [IA]=0X%016"PRIX64" [SA]=0X%04X [MY]=%05u,0X%04X [DEV-ADDR]=0X%04X [SN]=0X%08X [DEV-TYPE]=%c%c%c%c%c%c%c%c\n", cId, ieee, sa, my, my, dev, sn, rb[44 + offs], rb[45 + offs], rb[46 + offs], rb[47 + offs], rb[48 + offs], rb[49 + offs], rb[50 + offs], rb[51 + offs]);

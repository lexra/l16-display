...
void ProcessLeaveRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	...
	printf("LEAVE_RSP (0X%04X), [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64", found=%d\n", cId, my, my, sa, ieee, pdesc[7]);
}
...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X80F0 == cId)
	{
		ProcessLeaveRsp(DataLen, &pbuf[21]);
		return;
	}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -D [Device Short Address] -M [Remove Children] -J [Rejoin] -S [SecurityEnable] -t [timeout]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -D0X796F -M1 -J1 -S0 -t6\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:T:t:D:d:M:m:J:j:S:s:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'd':
			...
			if ('0' != optarg[0])						PrintUsage();
			if ('X' != optarg[1] && 'x' != optarg[1])		PrintUsage();
			if ('X' == optarg[1])						res = sscanf(optarg, "%X", &da);
			else										res = sscanf(optarg, "%x", &da);
			if (-1 == res)								PrintUsage();
			da &= 0XFFFF;
			break;
		case 'M':
	...
	if (0 == (len = PackDesc(0X00F0, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &da, sizeof(unsigned short));
	frame[7] = (unsigned char)remove;
	frame[8] = (unsigned char)rejoin;
	frame[9] = (unsigned char)security;

	if (0 == PackAfCmd((unsigned short)addr, 0X00F0, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("LeaveReq (0X0124,0X00F0): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for ADC_RSP (0X8009) TIMEOUT !\n\n", t);

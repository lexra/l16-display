...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -S [Source] -D [Destination]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X796F -r30 -o0X20 -S0X0001 -D0X0000\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	int offs = 0;
	unsigned int sn;
	int src = 0X0001, dst = 0X0000, dev = 0X0000;
	unsigned short cs;
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:s:S:d:D:")) != -1)
	{
		switch (c)
		{
		default:
...
//
	...
	len = MkSmaDataTelegram(telegram, src, dst, 0X80, 0X00, 0X06, 0, 0);
	...
	printf("SMA_GET_NET_START (0X0124,0X00EA): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TXOPTS]=0X%02X [SRC]=0X%04X [DST]=0X%04X\n", addr, radius, radius, txopts, src, dst);
...
//
	...
	do
	{
		...
		{
			if ((int)(now - first) > t)							printf("\nWAITING %d SECONDS for SMA_GET_NET_START_RSP (0X80E2) TIMEOUT !\n\n", t), 	close(sd), exit(1);
		}
		FD_ZERO(&rset);
		FD_SET(sd, &rset);
		...
		// fcs
		//memcpy(&cs, &rb[29 + 19 + 12 + offs], sizeof(unsigned short));
		cs = BUILD_UINT16(rb[29 + 19 + 13 + offs], rb[29 + 19 + 12 + offs]);
	...
	memcpy(&sn, &rb[40 + offs], sizeof(unsigned int));

	printf("\nSMA_GET_NET_START_RSP (0X%04X), [IA]=0X%016"PRIX64" [SA]=0X%04X [MY]=%05u,0X%04X [DEV-ADDR]=0X%04X [SN]=0X%08X [DEV-TYPE]=%c%c%c%c%c%c%c%c\n", cId, ieee, sa, my, my, dev, sn, rb[44 + offs], rb[45 + offs], rb[46 + offs], rb[47 + offs], rb[48 + offs], rb[49 + offs], rb[50 + offs], rb[51 + offs]);

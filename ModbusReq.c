...
{
	unsigned short cId = 0, my = 0XFFF0, gId = 0XFFF0;
	unsigned long long int ia = 0;
	unsigned short pan = 0XFFF0;
	unsigned char lqi = 0X00, sec = 0;
	...
	memcpy(&pan, &pbuf[18], sizeof(unsigned short));
	memcpy(&lqi, &pbuf[22], sizeof(char));
	br = pbuf[21];
	sec = pbuf[23];
	memcpy(&ts, &pbuf[24], sizeof(unsigned long));
	sn = pbuf[28];
	DataLen = pbuf[29];
	len = pbuf[1];
	...
	{
		if (0 != lqi && 0XFF != lqi)
			printf("AF_EXT_INCOMING (0X8244), [cId]=0X%04X [M]=0X%02X [SA]=0X%04X [gId]=0X%04X [br]=%u [Lqi]=%03u,0X%02X [sec]=%u [SN]=%03u [len]=%03u [plen]=%03u [ts]=%06u\n", 
				cId, m, (int)(ia & 0XFFFF), gId, br, lqi, lqi, sec, sn, len, DataLen, ts);
		else
			printf("AF_EXT_INCOMING (0X8244), [cId]=0X%04X [M]=0X%02X [SA]=0X%04X [gId]=0X%04X [br]=%u [Lqi]=N/A,0X00 [sec]=%u [SN]=%03u [len]=%03u [plen]=%03u [ts]=%06u\n", 
				cId, m, (int)(ia & 0XFFFF), gId, br, sec, sn, len, DataLen, ts);
	}
	else
	{
		if (0 != lqi && 0XFF != lqi)
			printf("AF_EXT_INCOMING (0X8244), [cId]=0X%04X [M]=0X%02X [IA]=0X%016"PRIX64" [gId]=0X%04X [br]=%u [Lqi]=%03u,0X%02X [sec]=%u [SN]=%03u [len]=%03u [plen]=%03u [ts]=%06u\n", 
				cId, m, ia, gId, br, lqi, lqi, sec, sn, len, DataLen, ts);
		else
			printf("AF_EXT_INCOMING (0X8244), [cId]=0X%04X [M]=0X%02X [IA]=0X%016"PRIX64" [gId]=0X%04X [br]=%u [Lqi]=N/A,0X00 [sec]=%u [SN]=%03u [len]=%03u [plen]=%03u [ts]=%06u\n", 
				cId, m, ia, gId, br, sec, sn, len, DataLen, ts);
	}
	...
	if (0X8018 == cId)
	{
		...
		((unsigned char *)&ieee)[0] = pbuf[21 + 9];
		((unsigned char *)&ieee)[1] = pbuf[22 + 9];
		((unsigned char *)&ieee)[2] = pbuf[23 + 9];

		memcpy(&my, &pbuf[26 + 9], sizeof(unsigned short));

		if (2 == m)
			printf("MODBUS_RSP (0X%04X), [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64"\n", cId, my, my, (int)(ia & 0XFFFF), ieee);
		else
			printf("MODBUS_RSP (0X%04X), [MY]=%05u,0X%04X [IA]=0X%016"PRIX64"\n", cId, my, my, ieee);

		memcpy(&start, &pbuf[28 + 9], sizeof(unsigned short));
		printf("[START REG]=0X%04X ", start);
		slave = pbuf[30 + 9];
		printf("[SLAVE]=0X%02X ", slave);
		fn = pbuf[31 + 9];
		printf("[FN]=0X%02X ", fn);
		bc = pbuf[32 + 9];
		printf("[BC]=%02u,0X%02X \n", bc, bc);
		...
		for (i = 0; i < (int)bc + 5; i++)
		{
			if (i == (int)bc + 5 - 1)
				...
				printf("[485-OUTPUT]: 0X%02X-", pbuf[30 + 9 + i]);
			else
				printf("0X%02X-", pbuf[30 + 9 + i]);
		}

		for (i = 0; i < bc / 2; i++)
		{
			w = BUILD_UINT16(pbuf[43 + 2 * i], pbuf[42 + 2 * i]);
			printf("[R-0X%04X]=%05u,0X%04X\n", start + i, w, w);
		}
	...
	printf("Unprocess AF_EXT_INCOMING, cId=0X%04X\n", cId);
	return;
}

// 0xfe Len Cmd0 Cmd1 Gid0 Gid1 Cid0 Cid1 Sa0 Sa1 Sep Dep Brocast Lqi Security Stamp0 Stamp1 Stamp2 Stamp3 Seq DataLen Data0 Data1 Data2 Data3 Data4 My0 My1 ...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0 != lqi && 0XFF != lqi)
		printf("AF_INCOMING (0X8144), [cId]=0X%04X [SA]=0X%04X [gId]=0X%04X [br]=%u [Lqi]=%03u,0X%02X [sec]=%u [SN]=%03u [len]=%03d [plen]=%03u [ts]=%06u\n", 
			cId, sa, gId, br, lqi, lqi, sec, sn, len, DataLen, ts);
	else
		printf("AF_INCOMING (0X8144), [cId]=0X%04X [SA]=0X%04X [gId]=0X%04X [br]=%u [Lqi]=N/A,0X00 [sec]=%u [SN]=%03u [len]=%03d [plen]=%03u [ts]=%06u\n", 
	...
	if (0X8018 == cId)
	{
		...
		for (i = 0; i < (int)bc + 5; i++)
		{
			if (i == (int)bc + 5 - 1)
				...
				printf("[485-OUTPUT]: 0X%02X-", pbuf[30 + i]);
			else
		...
		for (i = 0; i < bc / 2; i++)
		{
			...
			w = BUILD_UINT16(pbuf[34 + 2 * i], pbuf[33 + 2 * i]);
			printf("[R-0X%04X]=%05u,0X%04X\n", start + i, w, w);
		}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -s [Slave Address] -R [Start Register] -n [Number] -t [timeout]\n", filename);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -s0X01 -R0X0092 -n0X001A -t6\n", filename), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	SYSTEMTIME st;
	unsigned long long int a = 0X00124B000133B481LL;
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:s:S:n:N:o:O:T:t:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(MODBUSQRY_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)slave;
	frame[6] = (unsigned char)3;
	frame[7] = HI_UINT16((unsigned short)reg);
	frame[8] = LO_UINT16((unsigned short)reg);
	frame[9] = HI_UINT16((unsigned short)num);
	frame[10] = LO_UINT16((unsigned short)num);

	if (0 == PackAfCmd((unsigned short)addr, MODBUSQRY_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("> \n");
	for (i = 0; i < len; i++)	if (i == len - 1)	printf("%02X\n", frame[i]);	else	printf("%02X-", frame[i]);
	time(&first);
	GetLocalTime(&st);
	printf("[%04d-%02d-%02d %02d:%02d:%02d %010u]\n", st.tm_year, st.tm_mon + 1, st.tm_mday, st.tm_hour, st.tm_min, st.tm_sec, (unsigned int)first);
	printf("ModbusReq (0X0124,0X0018): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [SLAVE]=%u [FUNC]=3 [START-REG]=0X%04X [NUM]=0X%04X,%05u\n", addr, radius, radius, txopts, slave, reg, num, num);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for MODBUS_RSP (0X%04X) TIMEOUT !\n\n", t, MODBUSQRY_REQ_DESC | 0X8000);
		...
		if ((MODBUSQRY_REQ_DESC | 0X8000) != cId)
		{
			printf(".");
			continue;
		}
		...
		if (cmd == 0X8144)
		{
			memcpy(&r, &rb[28], sizeof(unsigned short));
			if (r != reg)
			{
				printf(".");
				continue;
		...
		if (cmd == 0X8244)
		{
			memcpy(&r, &rb[28 + 9], sizeof(unsigned short));
			if (r != reg)
			{
				printf(".");
				continue;
		...
		if (cmd == 0X8244)
		{
			memcpy(&a, &rb[9], sizeof(unsigned long long int));
			if (2 != rb[8])
			{
				printf(".\n");
				continue;
			}
			if ((int)(a & 0XFFFF) != (int)addr)
			{
				printf(".\n");
				continue;
			}

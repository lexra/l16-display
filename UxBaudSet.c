...
/*
	...
	{
		printf("UX_GCR_RSP, [MY]=0X%04X, [PORT]=%d, [VALUE]=0X%02X \n", my, pbuf[28], pbuf[29]);
		return;
	}
*/
	if (0X802D == cId || 0X802E == cId)
	{
		...
		printf("UX_BAUD_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64", [PORT]=%d, [VALUE]=0X%02X \n", my, my, sa, ieee, pbuf[28], pbuf[29]);
		return;
	}
...
{
	printf("Usage: UxBaudSet -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -v [value]\n");
	printf("Example: UxBaudSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X3B\n");
	printf("Example: UxBaudSet -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0XD8\n"), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(UXBAUD_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;
	frame[6] = (unsigned char)value;

	if (0 == PackAfCmd((unsigned short)addr, UXBAUD_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for UXBAUD_RSP (0X%04X,0X%04X) TIMEOUT !\n\n", t, UXBAUD_REQ_DESC | 0X8000, UXBAUD_SET_DESC | 0X8000);

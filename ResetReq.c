...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (RESET_RSP_DESC == cId)
	{
		...
		printf("RESET_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64"\n", my, my, sa, ieee);
		return;
	}
...
{
	printf("Usage: ResetReq -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -d [Delay Seconds]\n");
	printf("Example: ResetReq -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -d10\n"), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:s:S:d:D:o:O:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'd':
			delay = atoi(optarg);
			if (delay < 0 || delay > 60)				PrintUsage();
			break;
	...
	if (0 == (len = PackDesc(RESET_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &delay, 2);


	if (0 == PackAfCmd((unsigned short)addr, RESET_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("ResetReq (0X0124,0X0019): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [DELAY]=%d\n", addr, radius, radius, txopts, delay);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for RESET_RSP (0X%04X) TIMEOUT !\n\n", t, RESET_REQ_DESC | 0X8000);

...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if ((PWM_SET_DESC | 0X8000) == cId || (PWM_REQ_DESC | 0X8000) == cId)
	{
		...
		printf("PWM_RSP (0X%04X), [SA]=0X%04X [IA]=0X%016"PRIX64" [MY]=%05u,0X%04X [PwmValue]=%d\n", cId, sa, ieee, my, my, BUILD_UINT16(pbuf[28], pbuf[29]));
		return;
	}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -v [PwmValue] -t [timeout]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -v1250 -t6\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:V:v:T:t:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(PWM_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &value, 2);

	if (0 == PackAfCmd((unsigned short)addr, PWM_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("%s (0X0124,0X0002): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [PwmValue]=%d\n", filename, addr, radius, radius, txopts, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for PWM_RSP (0X8002,0X8003) TIMEOUT !\n\n", t);

...
void ProcessPxDirRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	printf("PXDIR_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64", port=0X%02X value=0X%02X \n", my, my, ieee, P, V);
...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -N [Pin Number] -V [In/Out (0/1)]\n", file);
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(PIN_DIR_SET_DESC, frame)))
	...
	if (0 == PackAfCmd((unsigned short)addr, PIN_DIR_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PinDirSet (0X0124,0X009B): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [PIN NUMBER]=%d [VALUE]=%d\n", addr, radius, radius, txopts, ioport, pin, value);

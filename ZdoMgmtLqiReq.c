...
void ProcessZdoMgmtLqiRsp(unsigned short CMD, unsigned char *pdesc)
{
	unsigned short cmd;
	...
	for (; i < cnt; i++, p += 22)
	{
		...
		printf("NB[%d], [IA]=0X%016"PRIX64" [SA]=0X%04X, [DP]=%d [LQI]=%03d\n", (int)start + i, ia, (int)sa, (int)dp, (int)lqi);
	}
}
...
int main(int argc, char *argv[])
{
	...
	printf("ZdoMgmtLqiReq (0X3125): [SHORT ADDR]=0X%04X [START INDEX]=%02u \n", addr, start);
	...
	do
	{
		...
		time(&now);
		if ((int)(now - first) > 12)
			printf("\nWAITING 12 SECONDS for ZDO_MGMT_LQI_RSP TIMEOUT !\n\n"), close(sd), exit(1);

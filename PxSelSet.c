...
void ProcessPxSelRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	printf("PXSEL_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64", port=0X%02X value=0X%02X \n", my, my, ieee, P, V);
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(PXSEL_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;


	if (0 == PackAfCmd((unsigned short)addr, PXSEL_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PxSelSet (0X0124,0X0025): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [VALUE]=0X%02X\n", addr, radius, radius, txopts, ioport, value);
	...
	do
	{
		...
		cId &= 0XFFFF;
		if ((PXSEL_REQ_DESC | 0X8000) != cId && (PXSEL_SET_DESC | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

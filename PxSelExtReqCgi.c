...
/////////////////////////////
// 
	if (0 == (l = PackDesc(PXSEL_REQ_DESC, frame)))
	{
	...
	if (0 == PackAfExtCmd(3, addr, (unsigned short)pan, PXSEL_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)l, frame))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!\n", __FILE__, __LINE__);
		goto END;
	}

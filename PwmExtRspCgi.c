...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X80B1 == cId)
	{
		...
		fprintf(cgiOut, "PWM_DETECT_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64"\n", my, my, ieee);
...
// jc note, ext
	...
	cid &= 0XFFFF;
	if (cid != PWM_REQ_DESC && cid != PWM_SET_DESC)
	{
	...
	{
		memset(temp, 0, sizeof(temp));
		memcpy(temp + 2 * 0, packet + 3 * 29, 2);
		memcpy(temp + 2 * 1, packet + 3 * 28, 2);
		//memcpy(temp + 2 * 0, packet + 3 * (clen + 4), 2);
		//memcpy(temp + 2 * 1, packet + 3 * (clen + 3), 2);
...
// packet + 3 * 17
	...
	if (cid == PWM_SET_DESC)
		memcpy(&frame[5], &value, sizeof(unsigned short));

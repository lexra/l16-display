...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -I [IEEE address] -S [Start Index] -T [Type]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -S0 -T0\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:T:a:A:s:S:")) != -1)
	{
		switch (c)
		{
		default:
	...
	printf("ZdoIeeeAddrReq (0X0125): [TYPE]=%d [START_INDEX]=%d [SA]=0X%04X \n", type, start, addr);

	state = 0;
	time(&first);
	do
	{
		unsigned long long int ieee = 0;
		time_t now;

...
static void ProcessWhoamiRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	fprintf(cgiOut, "WHOAMI_RSP, [MY]=%05u,0X%04X \n[SA]=0X%04X \n[IA]=0X%016"PRIX64" \n[PA]=0X%04X \n[PROFILE_ID]=0X%04X \n[PAN]=0X%04X \n[CH]=%04d \n[STACK_PROFILE]=%s \n[TYPE]=%s \n[LQI]=%d,0X%02X \n[RSSI]=%d \n", 
		my, my, sa, ia, pa, prof, pan, ch, SPF, type, lqi, lqi, (rssi > 127) ? (rssi - 256) : rssi);
}
...
// ZDO_NWK_ADDR_REQ
	...
	memset(frame, 0, sizeof(frame));
	l = PackDesc(WHOAMI_REQ_DESC, frame);
	if (0 == PackAfCmd(sa, WHOAMI_REQ_DESC, (unsigned char)op, (unsigned char)ra, (unsigned char)l, frame))
	{

...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X8055 == cId)
	{
		...
		printf("PULSEOUT_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64", [PORT]=%d, [PIN]=%d\n", my, my, sa, ieee, pbuf[27], pbuf[28]);
		return;
	}
...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -N [Pin Number]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -N0\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:P:a:A:r:R:o:O:N:n:")) != -1)
	{
		switch (c)
		{
		default:
	...
	if (0 == (len = PackDesc(0X0055, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;
	frame[6] = (unsigned char)pin;


	if (0 == PackAfCmd((unsigned short)addr, 0X0055, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PulseOutReq (0X0124,0X0055): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [PIN NUMBER]=%d\n", addr, radius, radius, txopts, ioport, pin);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for PulseOut_RSP (0X%04X) TIMEOUT !\n\n", t, 0X0055 | 0X8000);
				close(sd), exit(1);
			}
		}

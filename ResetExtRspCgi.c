...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	{
		memcpy(&my, &pbuf[26], sizeof(unsigned short));
		fprintf(cgiOut, "RESET_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" \n", my, my, ieee);
		return;
	}
...
// jc note, ext
	...
	if (mode != 3)
	{
		res = 1, fprintf(cgiOut, "(%s %d) mode error !!", __FILE__, __LINE__);
		goto END;
	}
...
// packet + 3 * 17
	...
	memcpy(&frame[5], &delay, sizeof(unsigned short));
	...
	if (l <= 0)
	{
		res = 1, fprintf(cgiOut, "(%s %d) PackFrame() fail !", __FILE__, __LINE__), close(sd);
		goto END;
	}
	pb = frame;
	left = l;

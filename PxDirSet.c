...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port]  -a [Short Address] -r [Radius] -o [Tx Options] -P [IO Port Number] -V [Value]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -P0 -v0X00\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(PXDIR_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	frame[5] = (unsigned char)ioport;


	if (0 == PackAfCmd((unsigned short)addr, PXDIR_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("PxDirSet (0X0124,0X0038): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [IO PORT NUMBER]=%u [VALUE]=0X%02X\n", addr, radius, radius, txopts, ioport, value);
	...
	do
	{
		...
		cId &= 0XFFFF;
		if ((PXDIR_REQ_DESC | 0X8000) != cId && (PXDIR_SET_DESC | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

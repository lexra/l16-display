...
static void ProcessPxDirRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	memcpy(&my, &pdesc[5], sizeof(unsigned short));
	fprintf(cgiOut, "PXDIR_RSP, [MY]=0X%04X, value=0X%02X \n", my, pdesc[8]);
...
static void ProcessPxSelRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	memcpy(&my, &pdesc[5], sizeof(unsigned short));
	fprintf(cgiOut, "PXSEL_RSP, [MY]=0X%04X, value=0X%02X \n", my, pdesc[8]);
...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X802F == cId || 0X8030 == cId)
	{
		...
		fprintf(cgiOut, "UX_GCR_RSP, [MY]=0X%04X, [IOPORT]=%d, [VALUE]=0X%02X \n", my, pbuf[28], pbuf[29]);
		return;
	}
	...
	{
		memcpy(&my, &pbuf[26], sizeof(unsigned short));
		fprintf(cgiOut, "UX_BAUD_RSP, [MY]=0X%04X, [IOPORT]=%d, [VALUE]=0X%02X \n", my, pbuf[28], pbuf[29]);
		return;
	}
	...
	if (0X8018 == cId)
	{
		...
		memcpy(&my, &pbuf[26], sizeof(unsigned short));
		fprintf(cgiOut, "MODBUS_QRY_RSP, [MY]=0X%04X \n", my);
		...
		bc = pbuf[32];
		fprintf(cgiOut, "[BC]=0X%02X \n", bc);

		for (i = 0; i < (int)bc + 5; i++)
		{
			if (i == (int)bc + 5 - 1)
				fprintf(cgiOut, "0X%02X \n", pbuf[30 + i]);
			else if (i == 0)
				fprintf(cgiOut, "[OUTPUT]: 0X%02X-", pbuf[30 + i]);
			else
				fprintf(cgiOut, "0X%02X-", pbuf[30 + i]);
		}
...
*/
	...
	{
		memcpy(&my, &pbuf[26], sizeof(unsigned short));
		fprintf(cgiOut, "PHOTO_RSP, [MY]=0X%04X \n[Value]=%02d\n", my, BUILD_UINT16(pbuf[28], pbuf[29]));
		return;
	}
...
/*
	...
	{
		ProcessModbusRsp(DataLen, &pbuf[21]);
		return;
	}

	if (PHONIX_READ_RSP_DESC == cId)
	{
		ProcessPhonixReadRsp(DataLen, &pbuf[21]);
		return;
	}

	if (PHONIX_READ_DESC_RSP_DESC == cId)
	{
		ProcessPhonixReadDescRsp(DataLen, &pbuf[21]);
		return;
	}

	if (NV_RSP_DESC == cId)
	{
		ProcessNvRsp(DataLen, &pbuf[21]);
		return;
	}
*/
...
// jc note, ext
	...
	res = sscanf(packet + 3 * 28, "%X", &ioport);
	if (-1 == res)
	{
	...
	res = sscanf(packet + 3 * 29, "%X", &value);
	if (-1 == res)
	{
...
// packet + 3 * 17
	...
	memset(frame, 0, sizeof(frame));
	l = PackDesc((unsigned short)cid, frame);
	if (0 == l)
	{
		res = 1, fprintf(cgiOut, "(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd);
		goto END;
	}
	frame[5] = (unsigned char)ioport;
	frame[6] = (unsigned char)value;

	if (0 == PackAfCmd(sa, (unsigned short)cid, (unsigned char)op, (unsigned char)ra, (unsigned char)l, frame))
	{
	...
	do
	{
		...
		if (rb[28] != (unsigned char)ioport)
		{
			fprintf(cgiOut, "(%s %d) ioport=%d, rb[28]=%d !\n", __FILE__, __LINE__, ioport, rb[28]);
			continue;
		}
		break;
	}

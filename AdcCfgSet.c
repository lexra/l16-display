...
void ProcessAdcCfgRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	unsigned char V;
	unsigned char i;
	...
	memcpy(&my, &pdesc[5], sizeof(unsigned short));
	printf("ADC_CFG_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64", value=0X%02X \n", my, my, ieee, pdesc[8]);

	V = pdesc[7];
	for (i = 0; i < 8; i++)
	{
		if (V & (1 << i))
			printf("P0_%d, Analog I/O ENABLED\n", i);
		else
			printf("P0_%d, Analog I/O DISABLED\n", i);
	}
}
...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	{
		ProcessAdcCfgRsp(DataLen, &pbuf[21]);
		return;
	}
...
void PrintUsage(void)
{
	...
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -v0X00\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(ADCCFG_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &value, 1);


	if (0 == PackAfCmd((unsigned short)addr, ADCCFG_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("AdcCfgSet (0X0124,0X002B): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [VALUE]=0X%02X\n", addr, radius, radius, txopts, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for ADCCFG_RSP (0X%04X,0X%04X) TIMEOUT !\n\n", t, ADCCFG_REQ_DESC | 0X8000, ADCCFG_SET_DESC | 0X8000);
		...
		cId &= 0XFFFF;
		if ((ADCCFG_REQ_DESC | 0X8000) != cId && (ADCCFG_SET_DESC | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

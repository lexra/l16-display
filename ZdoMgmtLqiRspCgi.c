...
static void ProcessZdoMgmtLqiRsp(unsigned short CMD, unsigned char *pdesc)
{
	unsigned short cmd;
	unsigned char status, tn, start = 0, cnt = 0;
	unsigned short sa;
	int i = 0;
	unsigned char *p;
	unsigned char dp, lqi;
	...
	fprintf(cgiOut, "\nZDO_MGMT_LQI_RSP, [CMD]=0X%02X \n[Status]=%s [SA]=0X%04X [TN]=%d \n", cmd, (0 == status) ? "SUCESS" : "FAIL", sa, tn);

	p = &pdesc[10];
	for (; i < cnt; i++, p += 22)
	{
		memcpy(&ia, p + 8, sizeof(unsigned long long int));
		memcpy(&sa, p + 16, sizeof(unsigned short));
		memcpy(&dp, p + 20, sizeof(unsigned char));
		memcpy(&lqi, p + 21, sizeof(unsigned char));

		fprintf(cgiOut, "NB[%d], [IA]=0X%016"PRIX64" [SA]=0X%04X, [DP]=%d [LQI]=%03d\n", (int)start + i, ia, (int)sa, (int)dp, (int)lqi);
	}
}
...
int cgiMain()
{
	...
	unsigned char hs[] = {0XFE, 0X00, 0X00, 0X04, 0XFA, 0X00};
...
// 
	...
	do
	{
		...
		if (addr != sa)
		{
			fprintf(cgiOut, "(%s %d) addr!=ia\naddr=0X%04X, sa=0X%04X\n", __FILE__, __LINE__, addr, sa);
			continue;
		}
		break;
	}

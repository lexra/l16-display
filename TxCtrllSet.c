...
int PackDesc(const unsigned short cId, unsigned char *pbuf)
{
	...
	if (GPIO_SET_DESC == cId)
		return 7;
	if (GPIO_REQ_DESC == cId || 0X001A == cId)
		return 6;
	if (ADC_REQ_DESC == cId)
		return 5;
...
void PrintUsage(void)
{
	...
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -v0XE5\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(0X001A, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &value, 1);

	if (0 == PackAfCmd((unsigned short)addr, 0X001A, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("%s (0X0124,0X001A): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [Value]=0X%02X\n", filename, addr, radius, radius, txopts, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for TXCTRLL_RSP (0X801A,0X801B) TIMEOUT !\n\n", t);
		...
		if ((0X001A | 0X8000) != cId && (0X001B | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

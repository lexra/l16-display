...
// radius check
	...
	ioport = atoi(temp);
	if (0 != ioport && 1 != ioport)
	{
		res = 1, fprintf(cgiOut, "(%s %d) ioport error !!\n", __FILE__, __LINE__);
		goto END;
	}
	memset(temp, 0, sizeof(temp));
	res = cgiFormStringNoNewlines("pin", temp, 24);
	if (cgiFormNotFound == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) pin not found !!\n", __FILE__, __LINE__);
		goto END;
	}
	if (strlen(temp) > 1)
	{
		res = 1, fprintf(cgiOut, "(%s %d) pin error !!\n", __FILE__, __LINE__);
		goto END;
	}
	pin = atoi(temp);
	if (pin < 0 || pin > 7)
...
// 
	...
	if (0 == (l = PackDesc(PIN_HI_LO_SET_DESC, frame)))
	{
	...
	if (0 == PackAfExtCmd(3, addr, (unsigned short)pan, PIN_HI_LO_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)l, frame))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!\n", __FILE__, __LINE__);
		goto END;
	}

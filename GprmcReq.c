...
void PrintUsage(void)
{
	...
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -t6\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	PackDesc(0X0101, frame);
		//printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, 0X0101, (unsigned char)txopts, (unsigned char)radius, (unsigned char)5, frame))
	...
	do
	{
		...
		{
			if ((int)(now - first) > t)
				printf("\nWAITING %d SECONDS for PGPRMC_RSP (0X8100,0X8101) TIMEOUT !\n\n", t), close(sd), exit(1);
		}

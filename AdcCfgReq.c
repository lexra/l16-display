...
void ProcessPxDirRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	for (i = 0; i < 8; i++)
	{
		if (V & (1 << i))			printf("P%d_%d=OUTPUT\n", P, i);
		else						printf("P%d_%d=INPUT\n", P, i);
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(ADCCFG_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);


	if (0 == PackAfCmd((unsigned short)addr, ADCCFG_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("AdcCfgReq (0X0124,0X002C): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);

 ...
 *   Ctrl-A P       Set the break period in deciseconds
 ...
 *   Ctrl-A Z       Toggle line buffered mode (readline and history)
...
void print_help(int command_line_usage)
{
	...
	if (command_line_usage) {
		...
		for (s = speeds; s->name; s++) {
			fprintf(stderr, "%s%s", s == speeds ? "  " : s->flag == B57600 ||
					s->flag == B1500000 ? ",\r\n  " : ", ", s->name);
		}
		fprintf(stderr, "\r\n\r\n");
	}
	...
	fprintf(stderr, "  Ctrl-%c Z       Toggle line buffered mode (readline and history)\r\n", escape_letter);
...
void chat_open(const char *command_string)
{
	...
	if (!fork())
	{
		...
		if (dup(0) < 0) {
			perror("dup in chat_open() failed");
			_exit(-1);
		}
...
char *readline_lastchar = NULL;
char *readline_lastline = NULL;

ssize_t read_char(int fd, void *buf)
{
	if (fd != STDIN_FILENO || !readline_mode)
		return read(fd, buf, 1);

	if (readline_lastline == NULL) {
		tcsetattr(STDIN_FILENO, TCSANOW, &oldkey_stdin);
		tcsetattr(STDOUT_FILENO, TCSANOW, &oldkey_stdout);
		readline_lastline = readline("-> ");
		tcsetattr(STDIN_FILENO, TCSANOW, &newkey);
		tcsetattr(STDOUT_FILENO, TCSANOW, &newkey);
		if (readline_lastline == NULL) {
			readline_mode = 0;
			fprintf(stderr, "\r\n[ Leaving readline mode ]\r\n");
			return read(fd, buf, 1);
		}
		readline_lastchar = readline_lastline;
		add_history(readline_lastline);
	}

	if (*readline_lastchar == 0) {
		*(char*)buf = '\r';
		free(readline_lastline);
		readline_lastline = NULL;
		readline_lastchar = NULL;
		return 1;
	}

	*(char*)buf = *(readline_lastchar++);
	return 1;
}
			...
			if (command || got_escape) {
				...
				if (c == 'b' || c == 'B') {
					if (break_period == 0) {
						...
						ioctl(to, TCSBRKP, break_period);
					}
					return 0;
				}
				if (c == 'p' || c == 'P') {
					static char periodstring[4096] = "";

					if (user_input(from, "Enter break period in deciseconds (0=default)", periodstring, 4096, argument))
						goto read_error;

					break_period = atoi(periodstring);
				...
				if (c == 'z' || c == 'Z') {
					readline_mode = 1;
					fprintf(stderr, "[ Entering readline mode (Ctrl-D to leave) ]\r\n");
					return 0;
				}
				if (c == 'u' || c == 'U') {
...
send_byte:
		...
		if (to == STDOUT_FILENO && chat_ptmx != -1) {
			if (write(chat_ptmx, &c, 1) != 1)
				perror("Single char write to chat_ptmx failed");
		}
		if (to == STDOUT_FILENO && net_active) {
			if (write(net_fd, &c, 1) != 1)
				perror("Single char write to net_fd failed");
		}
		if (to == comfd && local_echo) {
			if (write(STDOUT_FILENO, &c, 1) != 1)
				perror("Single char write to STDOUT_FILENO failed");
			if (c == '\r') {
...
int main(int argc, char *argv[])
{
	...
	while (!need_exit)
	{
		...
		} else if (readline_mode && readline_lastline) {
			FD_ZERO(&fds);
			FD_SET(STDIN_FILENO, &fds);
			ret = 1;
		} else {
		...
		if (ret == -1) {
			...
			if (FD_ISSET(comfd, &fds)) {
				...
				if (argc > 2) {
					unsigned char ch;
					if (read_char(STDIN_FILENO, &ch) == 1 && ch == 3) {
						fprintf(stderr, "\r\n** COMMANDS ABORT **\r\n");
						need_exit = -3;
					}
				} else if (chat_ptmx != -1) {

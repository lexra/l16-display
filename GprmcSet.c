...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if ((0X0100 | 0X8000) == cId || (0X0101 | 0X8000) == cId)
	{
		float x, y;
		...
		memcpy(&x, &pbuf[28], sizeof(float));
		memcpy(&y, &pbuf[32], sizeof(float));
		printf("GPRMC_RSP (0X%04X), [SA]=0X%04X [IA]=0X%016"PRIX64" [MY]=%05u,0X%04X [X]=%f [Y]=%f\n", cId, sa, ieee, my, my, x, y);
		return;
	}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -Y [latitude] -X [longitude] -t [timeout]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -Y25.006752 -X121.484238 -t6\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:t:T:X:x:Y:y:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'Y':
		case 'y':
			res = sscanf(optarg, "%f", &y);
			if (-1 == res)								PrintUsage();
			break;
		case 'X':
		case 'x':
			res = sscanf(optarg, "%f", &x);
			if (-1 == res)								PrintUsage();
			break;
	...
	PackDesc(0X0100, frame);
		//printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &y, sizeof(float));
	memcpy(&frame[9], &x, sizeof(float));

	if (0 == PackAfCmd((unsigned short)addr, 0X0100, (unsigned char)txopts, (unsigned char)radius, (unsigned char)5 + 4 + 4, frame))
	...
	printf("GprmcReq (0X0124,0X0101): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);
	...
	do
	{
		...
		{
			if ((int)(now - first) > t)
				printf("\nWAITING %d SECONDS for GPRMC_RSP (0X8100,0X8101) TIMEOUT !\n\n", t), close(sd), exit(1);
		}
		...
		if ((0X0101 | 0X8000) != cId && (0X0100 | 0X8000) !=cId)
		{
			printf(".");
			continue;
		}

...
void ProcessAdcRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	...
	n = pdesc[7];
	p = &pdesc[8];
	printf("ADC_RSP, [MY]=%05u,0X%04X [SA]=0X%04X [IA]=0X%016"PRIX64" \n", my, my, sa, ieee);
	for (; i < n; i++)
	{
		unsigned short v;

		memcpy(&v, p, sizeof(unsigned short)), p += 2;
		printf("[AD%d]=%05u,0X%04X \n", i, v, v);
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (0 == (len = PackDesc(ADC_REQ_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);

	if (0 == PackAfCmd((unsigned short)addr, ADC_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("AdcReq (0X0124,0X0009): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X\n", addr, radius, radius, txopts);

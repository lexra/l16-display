...
void ProcessGpioRsp(unsigned char descLen, unsigned char *pdesc)
{
	...
	memcpy(&my, &pdesc[5], sizeof(unsigned short));
	fprintf(cgiOut, "GPIO_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64", value=0X%02X \n", my, my, ieee, pdesc[8]);
...
// jc note, ext
	...
	res = sscanf(packet + 3 * 30, "%X", &value);
	//res = sscanf(packet + 3 * (clen + 5), "%X", &value);
	if (-1 == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) sscanf error !!\n", __FILE__, __LINE__);
		goto END;

...
static void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	if (0X8055 == cId)
	{
		...
		fprintf(cgiOut, "PULSEOUT_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64", [PORT]=%d, [PIN]=%d\n", my, my, ieee, pbuf[27], pbuf[28]);
		return;
	}
...
int cgiMain()
{
	...
	int op, ra, ioport = 0, pin = 0;//, value = -1;
...
// ia
	...
	if (clen != 0X1B)
	{
		res = 1, fprintf(cgiOut, "(%s %d) len error !!\n", __FILE__, __LINE__);
		goto END;
	}
...
// jc note, ext
	...
	//res = sscanf(packet + 3 * (clen + 3), "%X", &ioport);
	if (-1 == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) sscanf error !!\n", __FILE__, __LINE__);
		goto END;
	...
	res = sscanf(packet + 3 * 29, "%X", &pin);
	//res = sscanf(packet + 3 * (clen + 4), "%X", &pin);
	if (-1 == res)
	{
		res = 1, fprintf(cgiOut, "(%s %d) sscanf error !!\n", __FILE__, __LINE__);
		goto END;
...
/*
	...
	{
		res = 1, fprintf(cgiOut, "(%s %d) signal error !\n", __FILE__, __LINE__);
		goto END;
	}

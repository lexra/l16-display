...
// vmultim -h 192.168.3.30 -p 34000 -b 0X00 -x 640 -y 640 -w 0
...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -b [BUTTON] -x [X-COORDINATE] -y [Y-COORDINATE] -w [WHEEL]\n", file);
	printf("Example: %s -h192.168.3.30 -p34000 -b0 -x800 -y600 -w0\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:b:B:x:X:y:Y:w:W:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'w':
			...
			{
				if ('X' == optarg[1])				res = sscanf(optarg, "%X", &W);
				else								res = sscanf(optarg, "%x", &W);
				if (-1 == res)
					PrintUsage();
			}
			else
...
///////////////////////////////////////////////////////////
// HD HD CM0 CM1 0 0 B X0 X1 Y0 Y1 W CRC1 CRC0
...
// HD HD CM0 CM1 0 0 B X0 X1 Y0 Y1 W CRC1 CRC0
	...
	wb[0] = 0XFE, wb[1] = 0XFE;
	wb[2] = REPORTID_MOUSE, wb[3] = 0;
	wb[6] = B;
	memcpy(&wb[7], &X, sizeof(unsigned short));
	memcpy(&wb[9], &Y, sizeof(unsigned short));
	wb[11] = W;
	...
	printf("UPDATE_MOUSE: [B]=0X%02X [X]=0X%04X,%05u [Y]=0X%04X,%05u [W]=0X%02X,%d\n", B, X, X, Y, Y, W, W);
	printf("\n");
	return 0;
}

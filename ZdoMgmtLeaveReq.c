...
void ProcessZdoMgmtRtgRsp(unsigned short CMD, unsigned char *pdesc)
{
	unsigned short cmd;
	unsigned short sa;
...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [short address] -I [Device IEEE] -R [Rejoin/RemoveChildren]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -I0X00124B000133B481 -R0\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	int remove = 0;
	unsigned long long int da = 0X00124B000133B481LL;
	...
	while ((c = getopt(argc, argv, "h:p:a:A:S:s:I:i:R:r:")) != -1)
	{
		switch (c)
		{
		default:
		...
		case 'i':
		case 'I':
			if (strlen(optarg) != 18)				PrintUsage();
			if (optarg[1] == 'X')				res = sscanf(optarg, "%LX", &da);
			else								res = sscanf(optarg, "%Lx", &da);
			da &= 0XFFFFFFFFFFFFFFFFLL;
			break;


		case 'R':
		case 'r':
			if (strlen(optarg) != 1)				PrintUsage();
			remove = atoi(optarg);
			if (0 != remove && 1 != remove)		PrintUsage();
			break;
	...
	memcpy(&frame[5], &da, sizeof(unsigned long long int));
	frame[13] = (unsigned char)remove;
	len = PackFrame(frame);
	...
	printf("ZdoMgmtLeaveReq (0X3425): [SHORT ADDR]=0X%04X [DEV IEEE]=0X%016"PRIX64" [REJOIN]=%d \n", addr, da, remove);
	...
	do
	{
		...
		time(&now);
		if ((int)(now - first) > 36)
			printf("\nWAITING 12 SECONDS for ZDO_MGMT_LEAVE_RSP TIMEOUT !\n\n"), close(sd), exit(1);
	...
	printf("\nZDO_MGMT_LEAVE_RSP, [Status]=%s [SA]=0X%04X \n", (rb[6] == 0) ? "SUCCESS" : "FAIL", BUILD_UINT16(rb[4], rb[5]) );

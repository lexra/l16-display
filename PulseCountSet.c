...
void ProcessJboxRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	unsigned short v = 0XFFF0;
	unsigned short a = 0XFFF0;

	unsigned long long int ieee = 0X00124B000133B481LLU;
	...
	printf("JBOX_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" \n[DC VOLTAGE]=%05u \n[DC CURRENT]=%05u \n", my, my, ieee, v, a);
}

void ProcessNegTempRsp(unsigned char descLen, unsigned char *pdesc)
{
	unsigned short my = 0XFFF0;
	unsigned short v = 0XFFF0;
	...
	if (v > 80)
		printf("THERM_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" \n[THERM TEMPERATURE]=%02.1f ¢XC\n", my, my, ieee, (float)v / 10);
	else
		printf("THERM_RSP, [MY]=%05u,0X%04X [IA]=0X%016"PRIX64" \n[THERM TEMPERATURE]=%02.1f ¢XC\n", my, my, ieee, (float)v);
}
...
void ProcessAfIncoming(unsigned short cmd, unsigned char *pbuf)
{
	...
	{
		ProcessJboxRsp(DataLen, &pbuf[21]);
		return;
	}
	...
	if (PHOTO_RSP_DESC == cId)
	{
		...
		printf("PHOTO_RSP, [MY]=%05u,0X%02X [IA]=0X%016"PRIX64" \n[Value]=%02d\n", my, my, ieee, BUILD_UINT16(pbuf[28], pbuf[29]));
		return;
	}

	if (NTEMP_RSP_DESC == cId)
	{
		ProcessNegTempRsp(DataLen, &pbuf[21]);
		return;
	}
...
static int PackDesc(const unsigned short cId, unsigned char *pbuf)
{
	...
	if (WHOAMI_REQ_DESC == cId || NTEMP_REQ_DESC == cId || PULSE_COUNT_REQ_DESC == cId)
		return 5;
	if (PWM_SET_DESC == cId)
		return 7;
	if (PWM_REQ_DESC == cId)
...
static void PrintUsage(void)
{
	...
	printf("Usage: %s -h [host] -p [connect port] -a [Short Address] -r [Radius] -o [Tx Options] -v [Value]\n", file);
	printf("Example: %s -hlocalhost -p34000 -a0X0000 -r30 -o0X20 -v0\n", file), exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "h:p:a:A:r:R:o:O:V:v:")) != -1)
	{
		switch (c)
		{
		default:
	...
	//PackDesc(PULSE_COUNT_SET_DESC, frame);
	if (0 == (len = PackDesc(PULSE_COUNT_SET_DESC, frame)))
		printf("(%s %d) PackDesc() error !!\n", __FILE__, __LINE__), close(sd), exit(1);
	memcpy(&frame[5], &value, 4);

	//PackAfCmd((unsigned short)addr, PULSE_COUNT_SET_DESC, (unsigned char)txopts, (unsigned char)radius, 9, frame);
	if (0 == PackAfCmd((unsigned short)addr, PULSE_COUNT_SET_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)len, frame))
	...
	printf("%s (0X0124,0X%04X): [SA]=0X%04X [RADIUS]=0X%02X,%03u [TX OPTS]=0X%02X [VALUE]=%08lld\n", filename, PULSE_COUNT_SET_DESC, addr, radius, radius, txopts, value);
	...
	do
	{
		...
		else
		{
			if ((int)(now - first) > t)
			{
				printf("\nWAITING %d SECONDS for PULSE_COUNT_RSP (0X%04X) TIMEOUT !\n\n", t, PULSE_COUNT_SET_DESC | 0X8000);
		...
		if (addr == 0XFFFF)
		{
			if (cmd == 0X8144)
			{
				...
				if (1)
				{
					...
					memcpy(&cnt, &rb[28], sizeof(unsigned int));
					printf("PULSE_COUNT_RSP, [MY]=%05u,0X%02X [SA]=0X%04X [IA]=0X%016"PRIX64" ", my, my, sa, ieee);
					printf("[VALUE]=%08u \n", cnt);
				}
			}
			continue;

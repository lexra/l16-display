...
/////////////////////////////
// 
	if (0 == (l = PackDesc(ADC_REQ_DESC, frame)))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!\n", __FILE__, __LINE__);
		goto END;
	}

	if (0 == PackAfExtCmd(3, addr, (unsigned short)pan, ADC_REQ_DESC, (unsigned char)txopts, (unsigned char)radius, (unsigned char)l, frame))
	{
		res = 1, fprintf(cgiOut, "(%s %d) pack error !!\n", __FILE__, __LINE__);
		goto END;
	}
